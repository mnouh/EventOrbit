	...
	if(checkValues($_REQUEST['value']))
	{
		...
		echo "INSERT INTO facebook_posts (post,f_name,userip,date_created) VALUES('".checkValues($_REQUEST['value'])."','$model->firstName','".$userip."','".strtotime(date("Y-m-d H:i:s"))."')";
		
		mysql_query("INSERT INTO facebook_posts (post,f_name,userip,date_created) VALUES('".checkValues($_REQUEST['value'])."','$model->firstName','".$userip."','".strtotime(date("Y-m-d H:i:s"))."')");
	...
	while ($row = mysql_fetch_array($result))
	{
	   ...
	   <img src="<?php echo Yii::app()->request->baseUrl; ?>/images/empty-profile.jpg" style="float:left; width:50px; height:50px;" alt="" />
		   ...
		   <?php
			...
			<div class="commentBox" align="right" id="commentBox-<?php  echo $row['p_id'];?>" <?php echo (($comment_num_row) ? '' :'style="display:none"')?>>
				...
				<br clear="all" />
				<a id="SubmitComment" class="btn primary small comment"> Comment</a>
			</div>
	   </div>
	<?php
	}
